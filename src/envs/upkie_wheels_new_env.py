...
class UpkieWheelsNewEnv(UpkieBaseEnv):
    ...
    def __init__(
        self,
        config: Optional[dict] = DEFAULT_CONFIG,
        ...
        # position: 3 dim, velocity: 3 dim, height + pitch = 2 dim, 3 extra dim
        state_dim = 3 + 3 + 2 + 3
        state_max = np.hstack([q_max[:3], v_max[:3], height_pitch_max, extra_observation_limit])
        state_min = np.hstack([q_min[:3], -v_max[:3], height_pitch_min, -extra_observation_limit])
    ...
    def vectorize_observation(self, observation_dict: dict) -> np.ndarray:
        ...
        obs[8] = observation_dict["wheel_odometry"]["position"]
        obs[9] = observation_dict["wheel_odometry"]["velocity"]
        obs[10] = observation_dict["imu"]["angular_velocity"][1]
        return obs
...
class UpkieWheelsNewEnv2(UpkieBaseEnv):
    ...
    def __init__(
        ...
        action_dim = 6
        action_max = np.hstack([q_max[:3], v_max[:3]])
        action_min = np.hstack([q_min[:3], -v_max[:3]])
